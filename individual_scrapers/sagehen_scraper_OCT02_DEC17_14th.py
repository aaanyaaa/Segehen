...
# a 24hr clock.                                               
data_columns = [
    1, 3, 4, 5, 7, 8, 9, 11, 12, 13, 15, 16, 17, 19, 20, 22, 24, 25
]
...
def write_data(data_row, fieldnames):
    with open(output_file_name, 'a') as csv_file:
        ...
        hourlywriter.writerow({
            ...
            'SOIL TEMP AVE (Deg C)': data_row[9],
            'SOIL TEMP MAX (Deg C)': data_row[10],
            'SOIL TEMP MIN (Deg C)': data_row[11],
            'RELATIVE HUMIDITY AVE (%)': data_row[12],
            'RELATIVE HUMIDITY MAX (%)': data_row[13],
            'RELATIVE HUMIDITY MIN (%)': data_row[14],
            'DEW PT. (Deg C)': data_row[15],
            'WET BULB (Deg C)': data_row[16],
            'BARO. PRESS. (mb)': data_row[17],
            'SNOW DEPTH (mm)': data_row[18],
            'TOTAL PRECIP (mm)': data_row[19],
        })
        ...
        for month in range(1, 13):
            ...
            for day in range(1, 32):
                if year == 2002 and month <= 9:
                    pass
                elif year == 2017 and month == 12 and day > 14:
                    pass
                else:
                    try:
